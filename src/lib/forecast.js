...
export function buildModels(values, excluded, C){
  const d=values
  ...
  r.es={name:'Suavizado Exponencial',fitted:es,future:Array.from({length:6},(_,i)=>lastES*(1+lr.slope/Math.max(1,lr.intercept+lr.slope*d.length)*(i+1))),color:'#e37400'}
  r.lr={name:'Regresión Lineal',fitted:lr.fitted,future:lr.future,color:'#0b8043'}
  if(hw)r.hw={name:'Holt-Winters',fitted:hw.fitted,future:hw.future,color:'#7b1fa2'}
